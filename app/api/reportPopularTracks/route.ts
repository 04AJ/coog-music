...
export async function GET(req: NextRequest) {
    ...
    const trackRankings = await prisma.$queryRaw<Track[]>`
        ...
        FROM track, liked_tracks, artist, gender, genre, user as ua
        WHERE track.track_id = liked_tracks.track_id AND track.artist_id = artist.artist_id  AND ua.user_id = artist.user_id
                AND ua.gender_id = gender.gender_id AND gender.gender_id IN (${gender}) 
                AND track.genre_id = genre.genre_id AND track.genre_id IN (${genre}) 
                AND ua.ethnicity_id IN (${ethnicity})
