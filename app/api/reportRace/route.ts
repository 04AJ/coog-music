...
export async function GET(req: NextRequest) {
    ...
    const race = await prisma.$queryRaw<User[]>`
        SELECT race_name, COUNT(*) AS Total , ((COUNT(*) / (SELECT COUNT(*)
                                                            ...
                                                            WHERE LFA.artist_id = ${query} AND U.user_id = L.user_id AND LFA.listener_id = L.listener_id AND U.race_id = R.race_id)) * 100) AS PERCENT
        FROM race as R, user as U, listener_follows_artists as LFA, listener as L
        WHERE LFA.artist_id = ${query} AND U.user_id = L.user_id AND LFA.listener_id = L.listener_id AND U.race_id = R.race_id
        GROUP BY race_name;`;
